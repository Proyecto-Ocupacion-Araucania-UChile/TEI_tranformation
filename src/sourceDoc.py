...
class SourceDoc:
    ...
    def sourcedoc(self, document_name, root_tei, path):
        ...
        sourceDoc = etree.SubElement(root_tei, "sourceDoc")

        tags = ALTO.labels(path)
        ...
        input_alto_root = etree.parse(path).getroot()
        ...
        for tb in textblocks:
            ...
            for tl in textlines:
                ...
                if tl.id:
                    ...
                    if input_alto_root.find(f'.//a:TextLine[@ID="{tl.id}"]/a:String', namespaces=SourceDoc.NS).get(
                            "CONTENT") is not None \
                            and len(input_alto_root.find(f'.//a:TextLine[@ID="{tl.id}"]/a:String',
                                                         namespaces=self.SourceDoc.NS).getchildren()) == 0:
                    ...
                    # If the line's textual content is expressed at the level of glyphs, map that textual data to TEI element <c>.
                    elif input_alto_root.find(f'.//a:TextLine[@ID="{tl.id}"]/a:String', namespaces=SourceDoc.NS).get(
                            "CONTENT") is not None \
                            and input_alto_root.find(f'.//a:TextLine[@ID="{tl.id}"]/a:String',
                                                     namespaces=SourceDoc.NS).get(
                        "CONTENT") != "" \
                            and len(input_alto_root.find(f'.//a:TextLine[@ID="{tl.id}"]/a:String',
                                                         namespaces=SourceDoc.NS).getchildren()) > 0:

                        # Loop through all the <String> or <SP> children of a <TextLine>
                        textline_children = input_alto_root.find(f'.//a:TextLine[@ID="{tl.id}"]',
                                                                 namespaces=SourceDoc.NS).getchildren()
                        for textline_child in textline_children:
                            ...
                            if etree.QName(textline_child).localname == "SP":
                                ...
                                space_data = \
                                    attributes.zones(f'TextLine[@ID="{tl.id}"]', f'SP[@ID="{textline_child_id}"]',
                                                     None)[0]
                                strings_on_page += 1
                                surface_tree.zone3(textline, tb.id, tl.id, space_data.attributes, space_data.id,
                                                   strings_on_page)
                            ...
                            # If a child of <TextLine> is a segment of text <String>
                                ...
                                textline_child_id = textline_child.attrib["ID"]
                                string_data = \
                                    attributes.zones(f'TextLine[@ID="{tl.id}"]',
                                                     f'String[@ID="{textline_child_id}"]',
                                                     None)[0]
                                strings_on_page += 1
                                string = surface_tree.zone3(textline, tb.id, tl.id, string_data.attributes,
                                                            string_data.id, strings_on_page)

                                # Loop through all the <Glyph> children of a <String>
                                string_children = input_alto_root.findall(
                                    f'.//a:String[@ID="{textline_child_id}"]/a:Glyph', namespaces=SourceDoc.NS)
                                if words == "":
                                ...
                                for glyph_child in string_children:
                                    ...
                                    glyph_data = \
                                        attributes.zones(f'String[@ID="{textline_child_id}"]',
                                                         f'Glyph[@ID="{glyph_id}"]',
                                                         None)[0]
                                    glyphs_on_page += 1
                                    glyph = surface_tree.zone4(string, tb.id, tl.id, textline_child_id,
                                                               glyph_data.attributes, glyph_id, glyphs_on_page)
                                    surface_tree.car(glyph, glyph_child, tb.id, tl.id, textline_child_id, glyph_id,
                                                     glyphs_on_page)
